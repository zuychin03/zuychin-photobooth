...
Set `PB_RELEASE_MODE=local` for this release. Production also defaults to local mode when the setting is missing or unrecognised; development keeps the full acceptance environment unless local mode is explicitly selected. `PB_RELEASE_MODE=full` permits the remaining online features, and their individual feature switches still apply.

Local mode keeps the solo booth, editor, exports/sharing, device projects and templates available without Supabase or database migrations. Together also retains the existing live room flow: hosts create `/room/CODE?host=1`, and guests use `/room/CODE`. Cross-device rooms use the existing public Supabase URL/key for Realtime signalling and the existing optional TURN configuration; without Supabase, only same-browser tabs can pair. Accounts and new V2 database migrations are not required for this legacy room flow.

V2 room links (`?v=2`), relays, Events, cloud projects, Memories, accounts and the Shared Vault show a shared **Feature incoming** view. Direct links and client navigation use the same view. Account initialisation and cloud-save controls are disabled, and app APIs reject new V2 online work even if individual switches were accidentally enabled. Use a normal production build/start and HTTPS hosting; this is a limited friends release, not completion of V2's hosted acceptance.
